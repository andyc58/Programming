...
function love.load()
    ...
    player2 = Paddle(VIRTUAL_WIDTH - 15, VIRTUAL_HEIGHT - 30, 5, 35)
...
function love.update(dt)
    ...
    -- player 1 movement (AI Controlled paddle)
    if gameState == 'play' then 
        predicted_ball_x = player1.x + (player1.width / 2);
        
        --Calculates ball movement for the AI
        slope = ball.dy / ball.dx
        predicted_ball_y = slope * (predicted_ball_x - ball.x) + ball.y
        
        --Divides the AI paddle height in segments to prevent the paddle from bouncing
        paddle_segments = 6
        
        --Adds a small additional height for the AI paddle to hit the ball properly and prevents the paddle from bouncing
